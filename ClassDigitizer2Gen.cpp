...
void Digitizer2Gen::Initialization()
{
  ...
  for (int ch = 0; ch < MaxNumberOfChannel; ch++)
    chSettings[ch] = PHA::CH::AllSettings;
  ...
  for (int i = 0; i < (int)PHA::DIG::AllSettings.size(); i++)
    boardMap[PHA::DIG::AllSettings[i].GetPara()] = i;
  for (int i = 0; i < (int)PHA::LVDS::AllSettings.size(); i++)
    LVDSMap[PHA::LVDS::AllSettings[i].GetPara()] = i;
  for (int i = 0; i < (int)PHA::CH::AllSettings.size(); i++)
    chMap[PHA::CH::AllSettings[i].GetPara()] = i;
}
...
int Digitizer2Gen::FindIndex(const Reg para)
{
  ...
  {
  case TYPE::CH:
    return chMap[para.GetPara()];
  case TYPE::DIG:
    return boardMap[para.GetPara()];
  case TYPE::VGA:
    return 0;
  case TYPE::LVDS:
    return LVDSMap[para.GetPara()];
  case TYPE::GROUP:
    return 0;
  }
  return -1;
...
std::string Digitizer2Gen::ReadValue(const Reg para, int ch_index, bool verbose)
{
  ...
  {
  case TYPE::CH:
    chSettings[ch_index][index].SetValue(ans);
    break;
  case TYPE::DIG:
    boardSettings[index].SetValue(ans);
    break;
  case TYPE::VGA:
    VGASetting[ch_index].SetValue(ans);
    break;
  case TYPE::LVDS:
    LVDSSettings[ch_index][index].SetValue(ans);
    break;
  case TYPE::GROUP:
    InputDelay[ch_index].SetValue(ans);
    break;
  }
...
int Digitizer2Gen::OpenDigitizer(const char *url)
{
  ...
  else if (FPGAType == DPPType::PSD)
  {
    ...
    for (int ch = 0; ch < nChannels; ch++)
      chSettings[ch] = PSD::CH::AllSettings;
    for (int index = 0; index < 4; index++)
    {
    ...
    for (int i = 0; i < (int)PSD::DIG::AllSettings.size(); i++)
      boardMap[PSD::DIG::AllSettings[i].GetPara()] = i;
    for (int i = 0; i < (int)PSD::LVDS::AllSettings.size(); i++)
      LVDSMap[PSD::LVDS::AllSettings[i].GetPara()] = i;
    for (int i = 0; i < (int)PSD::CH::AllSettings.size(); i++)
      chMap[PSD::CH::AllSettings[i].GetPara()] = i;
  }
  else
  {
...
void Digitizer2Gen::SetDataFormat(unsigned short dataFormat)
{
  ...
  if (FPGAType == DPPType::PSD)
    ret = CAEN_FELib_GetHandle(handle, "/endpoint/dpppsd/stats", &stat_handle);
...
int Digitizer2Gen::ReadStat()
{
  ...
  if (ret != CAEN_FELib_Success)
    ErrorMsg("Read Statistics");

  for (int ch = 0; ch < nChannels; ch++)
    ReadValue(PHA::CH::SelfTrgRate, ch);
...
void Digitizer2Gen::PrintStat()
{
  ...
  for (int i = 0; i < nChannels; i++)
  {
    ...
    if (atoi(chSettings[i][0].GetValue().c_str()) == 0)
      continue;
...
int Digitizer2Gen::ReadData()
{
  ...
  if (FPGAType != DPPType::PHA && FPGAType != DPPType::PSD)
    return -404;
  ...
  if (hit->dataType == DataFormat::ALL)
  {
    if (FPGAType == DPPType::PHA)
    {
      ret = CAEN_FELib_ReadData(ep_handle, -1,
                                &hit->channel,
                                &hit->timestamp,
                                &hit->fine_timestamp,
                                &hit->energy,
  ...
  {
    ret = CAEN_FELib_ReadData(ep_handle, -1, hit->data, &hit->dataSize, &hit->n_events);
  ...
  printf("ch:%02d, energy: %d, trace Length %ld , event size %ld \n", hit->channel, hit->energy, hit->traceLength, hit->event_size);
  printf("Timestamp: %lu, fine timestamp: %d\n", hit->timestamp, hit->fine_timestamp);
...
void Digitizer2Gen::OpenOutFile(std::string fileName, const char *mode)
{
  ...
  sprintf(outFileName, "%s.bin", fileName.c_str());
...
void Digitizer2Gen::CloseOutFile()
{
  if (outFile != NULL)
  {
    ...
    if (result != 0)
      printf("somewrong when set file (%s) to read only.", outFileName);
  }
}
...
void Digitizer2Gen::SaveDataToFile()
{
  ...
  if (hit->dataType == DataFormat::ALL)
  {
    ...
    if (FPGAType == DPPType::PSD)
      fwrite(&hit->energy_short, 2, 1, outFile);
    ...
    fwrite(&hit->traceLength, 8, 1, outFile);
    // fwrite(hit->analog_probes_type,   2, 1, outFile);
    // fwrite(hit->digital_probes_type,  4, 1, outFile);
    // fwrite(hit->analog_probes[0], hit->traceLength*4, 1, outFile);
    // fwrite(hit->analog_probes[1], hit->traceLength*4, 1, outFile);
    // fwrite(hit->digital_probes[0], hit->traceLength, 1, outFile);
    // fwrite(hit->digital_probes[1], hit->traceLength, 1, outFile);
    // fwrite(hit->digital_probes[2], hit->traceLength, 1, outFile);
    // fwrite(hit->digital_probes[3], hit->traceLength, 1, outFile);
  }
  else if (hit->dataType == DataFormat::OneTrace)
  {
    fwrite(&dataStartIndetifier, 2, 1, outFile);
...
void Digitizer2Gen::ProgramBoard()
{
  ...
  if (FPGAType == DPPType::PSD)
    WriteValue("/par/EnStatEvents", "false");
...
void Digitizer2Gen::ProgramChannels()
{
...
void Digitizer2Gen::PrintBoardSettings()
{
  ...
  {
    if (boardSettings[i].ReadWrite() == RW::WriteOnly)
      continue;
  ...
  for (int i = 0; i < (int)LVDSSettings[0].size(); i++)
  {
    for (int index = 0; index < 4; index++)
    {
      if (LVDSSettings[index][i].ReadWrite() == RW::WriteOnly)
        continue;
...
void Digitizer2Gen::PrintChannelSettings(unsigned short ch)
{
  ...
  {
    if (chSettings[ch][i].ReadWrite() == RW::WriteOnly)
      continue;
...
void Digitizer2Gen::ReadAllSettings()
{
  ...
  if (ModelName != "VX2730")
  {
    if (CupVer >= 2023091800)
      for (int idx = 0; idx < 16; idx++)
        ReadValue(InputDelay[idx], idx, false);
  }
...
int Digitizer2Gen::SaveSettingsToFile(const char *saveFileName, bool setReadOnly)
{
  ...
  if (saveFile)
  {
    ...
    if (setReadOnly)
    {
      ...
      if (result != 0)
        printf("somewrong when set file (%s) to read only.", saveFileName);
    }
    ...
    while (fgets(line, sizeof(line), loadFile) != NULL)
    {
      ...
      while (token != nullptr)
      {
        ...
        *end = '\0';
        ...
        if (count == 0)
          std::strcpy(para, token);
        if (count == 1)
          std::strcpy(readWrite, token);
        if (count == 2)
          std::strcpy(idStr, token);
        if (count == 3)
      ...
      if (std::strcmp(readWrite, "2") == 0 && isConnected)
        WriteValue(para, value, false);
    }
...
std::string Digitizer2Gen::GetSettingValueFromMemory(const Reg para, unsigned int ch_index)
{
  ...
  case TYPE::CH:
    return chSettings[ch_index][index].GetValue();
  case TYPE::VGA:
    return VGASetting[ch_index].GetValue();
  case TYPE::LVDS:
    return LVDSSettings[ch_index][index].GetValue();
  case TYPE::GROUP:
    return InputDelay[ch_index].GetValue();
  default:
    return "invalid";
  }
